classdef HERBs_2AFC < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function main_OpeningFcn(app)
              ...
              if isfile('gui_settings_Decision_Making.mat')
                    loadStatus(app,'gui_settings_Decision_Making.mat');
              else
                    loadStatus(app,'gui_default_settings_Decision_Making.mat');
              end
        ...
        % Value changed function: retrial_mode
        function retrial_mode_Callback(app, event)
            if ~app.retrial_mode.Value
                app.training_reward.Value = 0;
            end
        end

        % Callback function
        function training_reward_Callback(app, event)
            
            if app.training_reward.Value
                app.retrial_mode.Value = 1;
            end
        end
        ...
        function figure1CloseRequest(app, event)
            % fully retract both spouts
              ...
              control_spouts(0,0,app);
              current_servo_position = [0,0];
              saveStatus(app,'gui_settings_Decision_Making.mat');
              delete(app);
%               delete(instrfindall);
...
%             
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.retrial_mode.ValueChangedFcn = createCallbackFcn(app, @retrial_mode_Callback, true);
